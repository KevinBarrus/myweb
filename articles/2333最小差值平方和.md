...
解释：假设差值分别为`[1,2,2,3]`且k=4，那么`gap[1]=1，gap[2]=2，gap[3]=1`

从最后一个元素开始，change=1，因此消耗1个单位，让差值为3的那个对子变成差值为2，因此`gap[2]`要加一，`gap[3]`要减一，k也要减一。也就是说，我们的贪心策略是：尽可能减小大差值出现的次数